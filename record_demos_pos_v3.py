"""
Scripted pick-and-place demo recorder — POSITION CONTROL v2.

Changes from v1:
  - N_SUBSTEPS=50  →  demos recorded at 10 Hz (matches inference & training, no downsampling needed)
  - State is 11-dim: [q0..q6, gripper_norm, ee_x, ee_y, ee_z]  (added end-effector XYZ)
  - Hold/settle step counts scaled down 10× to preserve wall-clock timing

Action space: [q0..q6 (right arm joint angle targets), gripper_norm]  (8-dim, unchanged)
State  space: [q0..q6, gripper_norm, ee_x, ee_y, ee_z]               (11-dim)

Output: data/pickplace_pos_v3/task_<N>/episode_NNNN.hdf5

Usage:
    python record_demos_pos_v2.py --task 0 --n-episodes 100
    python record_demos_pos_v2.py --task 0 --n-episodes 5 --no-viewer
"""
...
DATA_ROOT = REPO_ROOT / "data" / "pickplace_pos_v3"
...
# ── Camera / image ────────────────────────────────────────────────────────────
IMG_H, IMG_W = 224, 224
N_SUBSTEPS   = 50      # 10 Hz recording — matches inference (SUBSTEPS=50) directly
...
SETTLE_STEPS   = 5     # scaled from 50 (10Hz: same wall-clock ~0.5 s)
ABOVE_HEIGHT   = 0.14
LIFT_HEIGHT    = 0.14
...
def dls_ik_6d(model, data, site_id, target_pos, target_quat, q_mid):
    ...
    J_pos = J_pos[:, QVEL_RARM]
    J_rot = J_rot[:, QVEL_RARM]
...
def record_frame(renderer, data, site_id, imgs, wrists, states, actions,
    ...
    states.append(np.concatenate([
        ...
        [gripper_norm],
        data.site_xpos[site_id].astype(np.float32),   # EE xyz — new in v2
    ]))
...
def run_hold_phase(model, data, renderer, site_id, gripper_norm,
                   imgs, wrists, states, actions, n_steps=8, viewer=None):
...
def collect_episode(model, data, task_cfg: dict, renderer, viewer=None):
    ...
    run_cart_phase(model, data, renderer, site_id, above_tgt, q_mid,
                   ...
                   tol=0.05, timeout_steps=50, viewer=viewer)
    ...
    run_cart_phase_6d(model, data, renderer, site_id, grasp_tgt, target_quat,
                      ...
                      tol=0.010, timeout_steps=50, viewer=viewer)

    # Phase 3: close gripper (~0.8 s)
    run_hold_phase(model, data, renderer, site_id, 1.0, imgs, wrists, states, actions,
                   n_steps=8, viewer=viewer)
    ...
    # Phase 5: carry to target x (generous timeout — green arm config needs more steps)
    ...
    run_cart_phase(model, data, renderer, site_id, carry_tgt, q_mid,
                   ...
                   timeout_steps=300, viewer=viewer)
    ...
    # Phase 7: open gripper (~0.6 s)
    run_hold_phase(model, data, renderer, site_id, 0.0, imgs, wrists, states, actions,
                   n_steps=6, viewer=viewer)
...
def save_episode(out_dir, ep_idx, imgs, wrists, states, actions, success, prompt):
    ...
    with h5py.File(path, "w") as f:
        ...
        obs.create_dataset("state",       data=states)  # (T, 11)

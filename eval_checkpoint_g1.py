"""
G1 (single right arm) checkpoint evaluation script — closed-loop policy rollout via
serve_policy.py.

Adapted from eval_checkpoint.py (Baxter). Same structural difference as
eval_checkpoint_franka.py: G1's arm actuators are position-style (<position>,
kp=500), not Baxter's true velocity actuators, so a policy-output joint-position-
target action is applied directly to `ctrl` (clipped to joint ranges) instead of
being run through an outer P-controller to synthesize a velocity command.

Usage:
    python eval_checkpoint_g1.py \\
...
XML_PATH  = pathlib.Path(__file__).parent / "models" / "g1_twoblocks.xml"
...
def reset_scene(model, data, task_cfg):
    ...
    data.ctrl[CTRL_GRIP_L] = OPEN_L
    data.ctrl[CTRL_GRIP_R] = OPEN_R
...
def run_trial(client, model, data, renderer, task_cfg, joint_ranges):
    ...
    while t < MAX_STEPS:
        ...
        gripper_norm = ctrl_to_gripper_norm(data.ctrl[CTRL_GRIP_L])
        ...
        prev_gripper = ctrl_to_gripper_norm(data.ctrl[CTRL_GRIP_L])
        ...
        for _ in range(SUBSTEPS):
            ...
            data.ctrl[CTRL_GRIP_L] = gl
            data.ctrl[CTRL_GRIP_R] = gr
...
def main():
    ...
    parser.add_argument("--tasks",            default="0,1,2,4")
